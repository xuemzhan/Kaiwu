...
test('smoke: ActionRegistry actions have promptKey + temperature', () => {
  ...
  for (var i = 0; i < list.length; i++) {
    var a = list[i];
    assert.ok(
      typeof a.temperature === 'number' || a.temperature === undefined,
...
test('smoke: visibilitychange pauses/resumes polling timers', (t, done) => {
  ...
  setTimeout(function () {
    ...
    setTimeout(function () {
      ...
      // 清理 timer, 避免拖住事件循环
      if (ChatUI._actionCheckTimer) {
        clearInterval(ChatUI._actionCheckTimer);
        ChatUI._actionCheckTimer = null;
      }
      if (ChatUI._contextTimer) {
        clearInterval(ChatUI._contextTimer);
        ChatUI._contextTimer = null;
      }
      done();
    }, 50);
  }, 50);
...
test('smoke: AIService error classification', () => {
  ...
  for (var i = 0; i < tests.length; i++) {
    ...
    assert.ok(
      cls.message.indexOf(t.expectedContains) !== -1,
