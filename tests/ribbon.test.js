...
test('ribbon: OnAction routes to floating dialog for write action', () => {
  ...
  env.window.Application.PluginStorage.setItem = (k, v) => {
    if (k === 'pending_action') savedAction = v;
  };
  const r = loadRibbon(env.window);
...
test('ribbon: OnAction routes to settings for btnSettings', () => {
  ...
  r.TaskPaneManager.show = function () {
    showCalled = true;
  };
...
test('ribbon: OnAction handles btnOpenAssistant as sidebar toggle', () => {
  ...
  r.TaskPaneManager.toggle = function () {
    paneToggled = true;
    return true;
  };
...
test('ribbon: WakeWordManager.start attaches keydown listener', () => {
  ...
  env.window.Application.ShowDialog = () => {
    dialogOpened = true;
    return true;
  };
...
test('ribbon: WakeWordManager accepts uppercase Z key value without Shift', () => {
  ...
  assert.equal(
    dialogOpened,
    true,
    'uppercase key value should still trigger when Shift is not held'
  );
  env.window.WakeWordManager.stop();
});
...
test('ribbon: WakeWordManager opens assistant and preserves context when text is selected', () => {
  ...
  assert.equal(
    env.window.Application.PluginStorage.getItem('floating_selected_text'),
    'some selected text'
  );
  env.window.WakeWordManager.stop();
});
...
test('ribbon: WakeWordManager does not fire when meta is also held', () => {
  ...
  assert.equal(
    dialogOpened,
    false,
    'Ctrl+Alt+Meta+Z should not trigger (meta reserved for other bindings)'
  );
  env.window.WakeWordManager.stop();
});
...
test('ribbon: FloatingAssistantManager.show opens dialog and saves context', () => {
  const env = makeEnv();
  let dialogOpened = false;
  env.window.Application.ShowDialog = (url, title, w, h, modal) => {
    dialogOpened = true;
    return true;
  };
  const r = loadRibbon(env.window);
