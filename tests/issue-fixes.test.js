...
test('fix: streaming status with content renders the streamed text', async () => {
  ...
  await ResultCard.whenRendered();
  assert.ok(
    mount.innerHTML.indexOf('已生成部分内容') !== -1,
...
test('fix: streaming result has cancel button (■) instead of regenerate (↻)', async () => {
  ...
  assert.ok(
    html.indexOf('data-kw-action="abort"') !== -1,
...
test('fix: done result has regenerate button (↻) not cancel', () => {
  ...
  const html = mount.innerHTML;
  assert.ok(
    html.indexOf('data-kw-action="regenerate"') !== -1,
...
test('fix: ResultPanel.abort() marks active card as cancelled and clears controller', () => {
  ...
  assert.equal(ResultCard._cards[card.id].status, 'cancelled', 'card status should be cancelled');
...
test('fix: prepareUserAction can locate kwPrompt in floating dialog context', () => {
  ...
  assert.ok(
    prompt.placeholder && prompt.placeholder.length > 0,
    'placeholder should be set, got: ' + prompt.placeholder
  );
  assert.ok(
    prompt.placeholder.indexOf('仿写') !== -1 || prompt.placeholder.indexOf('风格样本') !== -1,
