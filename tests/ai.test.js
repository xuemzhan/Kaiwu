...
test('Config: isReasoningModel detects reasoning models by name', () => {
  ...
  assert.equal(
    env.window.Config.isReasoningModel('MiniMax-M3'),
    true,
    'MiniMax M3 should be detected as reasoning'
  );
...
test('AIService: sendStream missing apiKey calls onError', async () => {
  ...
  assert.ok(
    captured && captured.indexOf('API Key') !== -1,
    'expected API Key error, got: ' + captured
  );
});
...
test('AIService: sendStream sends correct request body and headers', async () => {
  ...
  assert.ok(
    captured.url.indexOf('/chat/completions') !== -1,
    'url should target chat completions: ' + captured.url
  );
  assert.equal(captured.opts.method, 'POST');
  assert.ok(
    captured.opts.headers['Authorization'].indexOf('sk-test') !== -1,
...
test('AIService: sendStream abort stops further processing', async () => {
  ...
  env.window.fetch = () =>
    Promise.resolve({ ok: true, status: 200, body: body, text: () => Promise.resolve(text) });
  const AI = loadAI(env.window);
  let chunks = 0;
...
test('AIService: buildMessages copies only role and content from history', () => {
  const env = makeEnv();
  const AI = loadAI(env.window);
  const history = [{ role: 'user', content: 'q1', extra: 'should be dropped' }];
