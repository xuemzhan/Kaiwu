...
test('ChatUI: streaming updates append to message', () => {
  ...
  const streamingMsg = container.querySelector('.streaming');
...
test('ChatUI: scenario tab switching updates active tab', () => {
  ...
  assert.ok(
    writingTab.classList.contains('is-active') ||
      writingTab.getAttribute('aria-selected') === 'true',
    'writing tab should be active by default'
  );
  const clickEvent = new window.Event('click', { bubbles: true });
  analysisTab.dispatchEvent(clickEvent);
  assert.ok(
    analysisTab.classList.contains('is-active') ||
      analysisTab.getAttribute('aria-selected') === 'true',
