...
test('package includes .env.example file', () => {
  ...
  assert.ok(
    content.includes('VITE_DEFAULT_API_KEY'),
    '.env.example should contain VITE_DEFAULT_API_KEY placeholder'
  );
  assert.ok(
    content.includes('PLEASE_REPLACE'),
    '.env.example should contain placeholder instruction'
  );
});
...
test('package includes README-安装说明.md', () => {
  ...
  assert.ok(
    content.includes('安装'),
    'README-安装说明.md should contain installation instructions'
  );
});
