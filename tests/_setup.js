...
function makeEnv(html) {
  ...
  const defaultApp = {
    ...
    PluginStorage: {
      getItem(k) {
        return Object.prototype.hasOwnProperty.call(pluginStore, k) ? pluginStore[k] : null;
      },
      setItem(k, v) {
        pluginStore[String(k)] = String(v);
      },
      removeItem(k) {
        delete pluginStore[k];
      },
    },
    ...
    GetTaskPane: function (id) {
      return taskPanes[id] || null;
    },
    ShowDialog: function () {
      return true;
    },
    ApiEvent: {
      ...
      fire: function (name, data) {
        (defaultApp._listeners[name] || []).forEach((fn) => {
          try {
            fn(data);
          } catch (e) {
            /* ignore */
...
function loadScripts(window, paths) {
  ...
  const prefix =
    'var window = this.window; var self = this.window; var globalThis = this.window;\n';
...
function mockVendorLibs(window) {
  ...
  window.marked = {
    ...
    parse: (text) => {
      ...
      html = html.replace(
        /!\[([^\]]*)\]\(([^)]+)\)/g,
        (m, alt, href) => '<img src="' + href + '" alt="' + alt + '">'
      );
      ...
      html = html
        .split(/\n\n+/)
        .map((p) =>
          /^<(h\d|pre|ul|ol|img|table|blockquote)/.test(p.trim()) ? p : '<p>' + p + '</p>'
        )
        .join('\n');
      return html;
    },
