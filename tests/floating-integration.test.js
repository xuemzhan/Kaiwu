...
test('floating: send button is wired and disabled on empty prompt', () => {
  ...
  assert.equal(
    sendBtn.classList.contains('is-ready'),
    false,
...
test('floating: send() with mocked fetch shows streamed answer', async () => {
  ...
  assert.ok(
    answer.innerHTML.indexOf('你好') !== -1,
    'answer should contain streamed content: ' + answer.innerHTML
  );
...
test('floating: send() with network error shows error message', async () => {
  ...
  assert.ok(
    answer.innerHTML.indexOf('network') !== -1 || answer.innerHTML.indexOf('kw-error') !== -1,
...
test('floating: send() with missing apiKey shows configuration error', async () => {
  ...
  const answer = env.window.document.getElementById('kwAnswer');
  assert.ok(
    answer.innerHTML.indexOf('API Key') !== -1 || answer.innerHTML.indexOf('请先') !== -1,
...
test('floating: dragging the composer (not titlebar) calls window.moveBy', () => {
  ...
  env.window.document.body.innerHTML = FLOATING_DOM;
  const moveByCalls = [];
  env.window.moveBy = (dx, dy) => {
    moveByCalls.push({ dx: dx, dy: dy });
  };
  loadFloating(env.window);
  triggerDOMContentLoaded(env.window);
  ...
  const downEv = new env.window.MouseEvent('mousedown', {
    clientX: 100,
    clientY: 50,
    bubbles: true,
  });
  ...
  env.window.document.dispatchEvent(
    new env.window.MouseEvent('mousemove', { clientX: 101, clientY: 51, bubbles: true })
  );
  assert.equal(moveByCalls.length, 0, 'tiny move should not trigger drag');
  // Move beyond threshold
  env.window.document.dispatchEvent(
    new env.window.MouseEvent('mousemove', { clientX: 130, clientY: 80, bubbles: true })
  );
  assert.ok(
    moveByCalls.length > 0,
    'large move should trigger drag, calls: ' + JSON.stringify(moveByCalls)
  );
...
test('floating: dragging on textarea is ignored (allows cursor positioning)', () => {
  ...
  const prompt = env.window.document.getElementById('kwPrompt');
  prompt.dispatchEvent(
    new env.window.MouseEvent('mousedown', {
      clientX: 80,
      clientY: 50,
      button: 0,
      bubbles: true,
  ...
  assert.equal(
    moveByCalls.length,
    0,
    'drag from textarea should be ignored (allow text selection)'
  );
  env.window.document.dispatchEvent(new env.window.MouseEvent('mouseup', { bubbles: true }));
});
...
test('floating: dragging is ignored when starting on a button', () => {
  ...
  btn.dispatchEvent(
    new env.window.MouseEvent('mousedown', { clientX: 100, clientY: 100, bubbles: true })
  );
  env.window.document.dispatchEvent(
    new env.window.MouseEvent('mousemove', { clientX: 200, clientY: 200, bubbles: true })
  );
...
test('floating: composer only contains prompt + 3 toolbar buttons (极简, 与 WPS 原生 AI 一致)', () => {
  ...
  assert.equal(
    buttons.length,
    4,
    'composer should have exactly 4 buttons (AI指令, +, 深入研究, 发送)'
  );
  ...
  assert.equal(
    env.window.document.querySelector('.kw-titlebar'),
    null,
    'no custom titlebar (matches WPS native AI)'
  );
});
...
test('floating dialog has brand mark element', () => {
  ...
  assert.equal(
    ...
    '开悟 AI 助手',
    'brand should have correct title attribute'
  );
});
...
test('floating brand mark has correct structure', () => {
  ...
  assert.equal(
    ...
    '../images/kaiwu.svg',
    'brand image should reference kaiwu.svg'
  );
...
test('floating: AI command panel respects vertical viewport bounds', async () => {
  ...
  assert.ok(
    top >= 8 && top <= 600 - 150 - 8,
    'panel top should be clamped within viewport, got: ' + top
  );
  assert.ok(
    left >= 8 && left <= 800 - 300 - 8,
    'panel left should be clamped within viewport, got: ' + left
  );
});
