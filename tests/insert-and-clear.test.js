...
test('insert: clicking 插入到文档 calls WriterAdapter.insertAtCursor', async () => {
  ...
  window.document.body.appendChild(drawer);
  let inserted = null;
  WriterAdapter.insertAtCursor = (text) => {
    inserted = text;
    return true;
  };
  let toast = null;
...
test('clear: card header has a 清除此条 button with danger styling', () => {
  ...
  assert.ok(
    mount.innerHTML.indexOf('title="删除"') !== -1,
...
test('logo: kaiwu.svg represents the 开悟 concept (book + light)', () => {
  ...
  assert.ok(
    /书页|书|page/i.test(content) || /path d=/.test(content),
  ...
  // 标题应包含 "开悟"
  assert.ok(
    content.indexOf('开悟') !== -1 || content.indexOf('Kaiwu') !== -1,
...
test('logo: kaiwu.svg opens successfully and is well-formed', () => {
  ...
  assert.ok(
    openTags >= closeTags,
    'opening tags should match closing (allowing self-closing paths)'
  );
});
