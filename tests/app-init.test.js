...
test('app: detects WPS bridge', () => {
  ...
  assert.equal(
    typeof env.window.__WPS_BRIDGE__.isWPSEnv,
    'function',
    'isWPSEnv should be a function'
  );
  assert.equal(
    typeof env.window.__WPS_BRIDGE__.getComponentType,
    'function',
    'getComponentType should be a function'
  );
  assert.equal(
    typeof env.window.__WPS_BRIDGE__.insertText,
    'function',
    'insertText should be a function'
  );
  assert.equal(
    typeof env.window.__WPS_BRIDGE__.readSelection,
    'function',
    'readSelection should be a function'
  );
  assert.equal(
    typeof env.window.__WPS_BRIDGE__.readFullContent,
    'function',
    'readFullContent should be a function'
  );
});
...
test('app: WPS bridge isWPSEnv returns correct value', () => {
  ...
  assert.equal(
    env.window.__WPS_BRIDGE__.isWPSEnv(),
    false,
    'should return false without Application'
  );
});
...
test('app: initializes ChatManager', () => {
  ...
  assert.ok(env.window.ChatManager, 'ChatManager should be exposed');
  assert.equal(
    typeof env.window.ChatManager.getCurrent,
    'function',
    'getCurrent should be a function'
  );
...
test('app: first-run detection works', () => {
  ...
  assert.equal(
    typeof env.window.FirstRunManager.dismiss,
    'function',
    'dismiss should be a function'
  );
...
test('app: ComponentDetector integration works', () => {
  ...
  assert.equal(
    typeof env.window.ComponentDetector.detect,
    'function',
    'detect should be a function'
  );
  assert.equal(
    typeof env.window.ComponentDetector.getLabel,
    'function',
    'getLabel should be a function'
  );
