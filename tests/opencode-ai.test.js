  ...
  return new Promise((resolve) => {
    OpenCodeAIService._request(
      ...
      (data) => {
        ...
        assert.ok(
          capturedFetch.options.headers.Authorization.startsWith('Basic '),
  ...
  return new Promise((resolve) => {
    OpenCodeAIService._request(
      ...
      (err) => {
        ...
        assert.ok(
          err.message.includes('Unauthorized') || err.message.includes('认证失败'),
...
test('OpenCodeAIService: sendStream parses SSE chunks', async () => {
  ...
  const { window, OpenCodeAIService, Config } = loadServiceWithMockFetch(() =>
    Promise.resolve(makeSSE(chunks))
  );
  Config.init();
...
test('OpenCodeAIService: abort cancels stream', async () => {
  ...
  const { window, OpenCodeAIService, Config } = loadServiceWithMockFetch(() =>
    Promise.resolve({ ok: true, status: 200, body: body })
  );
  Config.init();
  let chunks = 0;
...
test('OpenCodeAIService: abort does nothing when no stream active', () => {
  const { OpenCodeAIService } = loadServiceWithMockFetch(() =>
...
test('OpenCodeAIService: sendStream sends correct request body and headers', async () => {
  ...
  const { window, OpenCodeAIService, Config } = loadServiceWithMockFetch((url, opts) => {
    ...
    return Promise.resolve(
      makeSSE(['data: {"choices":[{"delta":{"content":"ok"}}]}', 'data: [DONE]'])
    );
  });
  Config.init();
  await new Promise((resolve) => {
  ...
  assert.ok(
    captured.url.indexOf('/session/test-session-123/message') !== -1,
    'url should target session message endpoint: ' + captured.url
  );
  assert.equal(captured.opts.method, 'POST');
  assert.ok(
    captured.opts.headers['Authorization'].indexOf('Basic') !== -1,
...
test('OpenCodeAIService: sendStream handles content field directly', async () => {
  const chunks = ['data: {"content":"direct content"}', 'data: [DONE]'];
  const { window, OpenCodeAIService, Config } = loadServiceWithMockFetch(() =>
    Promise.resolve(makeSSE(chunks))
  );
  Config.init();
...
test('OpenCodeAIService: mapDocumentAction handles doc_qa with query', () => {
  const { OpenCodeAIService } = loadService();
  const result = OpenCodeAIService.mapDocumentAction(
    'doc_qa',
    '文档内容在这里。',
    '文档的主要观点是什么？'
  );
  assert.equal(
    result.system,
    '你是一个文档问答助手。基于提供的文档内容回答用户问题。如果无法从文档中找到答案，请明确说明。'
  );
...
test('OpenCodeAIService: mapDocumentAction returns correct prompt for talk_doc', () => {
  ...
  assert.equal(
    result.system,
    '你是一个专业的文档讲解助手，擅长将书面内容转化为适合朗读的叙述性语言。'
  );
...
test('OpenCodeAIService: mapLayoutAction returns ai_layout prompt', () => {
  ...
  assert.ok(
    result.user.includes('标题、摘要、关键词'),
    'user prompt should mention paper sections'
  );
});
...
test('OpenCodeAIService: mapSpecializedAction returns correct prompt for legal', () => {
  ...
  assert.equal(
    result.system,
    '你是一个专业的法律助手，熟悉中国法律体系，擅长解答法律问题、解释法律条款、分析法律关系。请使用专业、严谨的法律语言，必要时引用相关法律条文。'
  );
...
test('OpenCodeAIService: mapSpecializedAction returns correct prompt for deep_think', () => {
  ...
  assert.equal(
    result.system,
    '你是一个深度分析助手。请深入分析问题，逐步推理，详尽考虑各方面因素，给出全面而有深度的回答。请直接给出分析结果。'
  );
...
test('OpenCodeAIService: mapSpecializedAction returns correct prompt for gen_image', () => {
  ...
  assert.equal(
    result.system,
    '你是一个图片描述生成助手。请将用户的描述转换为一个详细、生动的图片场景描述，适合用于AI绘图。'
  );
...
test('OpenCodeAIService: mapSpecializedAction returns correct prompt for summary_image', () => {
  const { OpenCodeAIService } = loadService();
  const result = OpenCodeAIService.mapSpecializedAction(
    'summary_image',
    '本文介绍了机器学习的基本概念和发展历史。'
  );
...
// =============================================================================
// _request: settled guard, JSON parse error classification, timeout isolation
// (added when the broken double-fire Promise.race pattern was rewritten to
// use a settled flag + fireSuccess/fireError helpers).
// =============================================================================

test('OpenCodeAIService: _request classifies JSON parse error as non-retryable', () => {
  // 2xx response but body is not valid JSON — the new code path calls
  // fireError with retryable: false (was retryable: true under the old
  // generic catch-all).
  const fetchMock = async () => ({
    ok: true,
    json: async () => {
  ...
  return new Promise((resolve) => {
    OpenCodeAIService._request(
      ...
      {},
      (data) => {
        assert.fail('onSuccess should not fire on JSON parse error');
      },
      (err) => {
        assert.ok(
          err.message.includes('JSON parse error'),
          'error message should mention JSON parse, got: ' + err.message
        );
        assert.equal(err.retryable, false, 'JSON parse errors must be non-retryable');
        resolve();
      }
    );
  });
...
test('OpenCodeAIService: _request fires each callback at most once even on slow body parse', () => {
  // Race scenario: fetch resolves quickly, but response.json() rejects
  // later. The settled guard ensures onError fires exactly once and
  // onSuccess never fires. The OLD code (Promise.race + chained .then
  // + .catch) could fire onError twice in this scenario.
  let successCount = 0;
  let errorCount = 0;
  const fetchMock = async () => ({
    ok: true,
    json: () =>
      new Promise((_, reject) => {
        setTimeout(() => reject(new SyntaxError('late parse failure')), 5);
      }),
  });
  const { OpenCodeAIService, Config } = loadServiceWithMock(fetchMock);
  Config.init();
  ...
  return new Promise((resolve) => {
    OpenCodeAIService._request(
      ...
      (err) => {
        errorCount++;
        // Wait a tick to let any spurious second callback fire.
        setTimeout(() => {
          assert.equal(successCount, 0, 'onSuccess should never fire');
          assert.equal(errorCount, 1, 'onError must fire exactly once, got ' + errorCount);
          resolve();
        }, 20);
      }
    );
...
test('OpenCodeAIService: _request clears the timeout on success (no leaked timers)', () => {
  // After a successful response, the 60s setTimeout must be cleared.
  // We can't directly inspect the timer ID (it lives in the closure),
  // but we can assert that subsequent calls work correctly without
  // pending timers stalling the event loop. We use a short timeout
  // so the test runs in <100ms even if a leak exists.
  ...
  global.setTimeout = function (fn, ms) {
    ...
    if (ms >= 100) {
      // This is the request-timeout timer (>=100ms in our tests).
      seen.add(id);
    }
    return id;
  };
  ...
  return new Promise((resolve) => {
    OpenCodeAIService._request(
      ...
      {},
      (data) => {
        // Give microtasks a chance to run, then assert no leaked timers.
        setTimeout(() => {
          ...
          assert.equal(
            seen.size,
            0,
            'expected 0 long-lived timers after success, got ' + seen.size
          );
          resolve();
        }, 50);
      },
...
test('OpenCodeAIService: _request does not log auth info by default; verbose=true enables it', () => {
  const originalLog = console.log;
  let logCount = 0;
  console.log = function (...args) {
    const s = args.join(' ');
    if (s.includes('[OpenCodeAI] Auth:')) logCount++;
  };
  ...
  return new Promise((resolve) => {
    // First call: no verbose → should NOT log
    OpenCodeAIService._request(
      'GET',
      '/session',
      null,
      {},
      () => {
        assert.equal(logCount, 0, 'no auth log when verbose is unset');

        // Second call: verbose=true → SHOULD log
        OpenCodeAIService._request(
          'GET',
          '/session',
          null,
          ...
          () => {
            ...
            assert.equal(logCount, 1, 'auth log fires exactly once when verbose=true');
            resolve();
          },
          (err) => assert.fail(err.message)
        );
