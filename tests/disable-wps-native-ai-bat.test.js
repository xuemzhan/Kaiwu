...
test('generateDisableNativeAiBat creates file placeholders with backup', () => {
  ...
  // Non-empty marker (DISABLED_BY_KAIWU) avoids WPS crashing on PE-load
  // when it tries to load the placeholder as a DLL.
  assert.ok(s.includes('DISABLED_BY_KAIWU'), 'must write a non-empty placeholder marker');
  assert.ok(
    !s.includes('type nul >'),
    'must NOT create a 0-byte placeholder (causes WPS crash on PE-load)'
  );
});
