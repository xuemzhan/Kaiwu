...
test('SessionManager: create generates session ID with correct format', async () => {
  ...
  env.window.fetch = async () => ({
    ok: true,
    json: async () => ({ id: 'ses-123', title: 'Test' }),
  });
...
test('SessionManager: get returns stored session', async () => {
  ...
  env.window.fetch = async () => ({
    ok: true,
    json: async () => ({ id: 'ses-456', title: 'Test2' }),
  });
  loadScripts(env.window, ['taskpane/services/config.js', 'taskpane/services/session-manager.js']);
...
test('SessionManager: clear removes session', async () => {
  ...
  env.window.fetch = async () => ({
    ok: true,
    json: async () => ({ id: 'ses-789', title: 'Test3' }),
  });
  loadScripts(env.window, ['taskpane/services/config.js', 'taskpane/services/session-manager.js']);
...
test('SessionManager: prune removes old sessions', async () => {
  ...
  sm._sessions['doc-old'] = {
    id: 'ses-old-1',
    documentId: 'doc-old',
    createdAt: Date.now() - 8 * 24 * 60 * 60 * 1000,
  };
  sm._sessions['doc-new'] = {
    id: 'ses-new-1',
    documentId: 'doc-new',
...
test('AppSessionManager: cleanup called on document close', async () => {
  ...
  env.window.fetch = async () => ({
    ok: true,
    json: async () => ({ id: 'ses-close-test', title: 'Close Test' }),
  });
  loadScripts(env.window, ['taskpane/services/config.js', 'taskpane/services/session-manager.js']);
  ...
  assert.strictEqual(
    typeof appSM._cleanupCurrentDocumentSessions,
    'function',
...
test('AppSessionManager: prune removes sessions older than maxAge', async () => {
  ...
  sm._sessions['doc-old-1'] = {
    id: 'ses-old-1',
    documentId: 'doc-old-1',
    createdAt: Date.now() - 10 * 24 * 60 * 60 * 1000,
  };
  sm._sessions['doc-old-2'] = {
    id: 'ses-old-2',
