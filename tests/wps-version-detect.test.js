...
test('detectInstalledWpsVersions detects at least one WPS on this dev machine', () => {
  ...
  const hasLocal = versions.some(
    (v) => v.cloudsvrPath && v.cloudsvrPath.toLowerCase().includes('appdata')
  );
...
test('detectInstalledWpsVersions does not include non-WPS paths', () => {
  ...
  for (const v of versions) {
    ...
    if (v.cloudsvrPath) {
      assert.ok(
        v.cloudsvrPath.includes('wpscloudsvr.exe'),
        'cloudsvrPath must reference wpscloudsvr.exe'
      );
    }
  }
});
