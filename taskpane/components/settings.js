...
var SettingsUI = {
  ...
  _isPresetModel: function (model) {
    for (var i = 0; i < PRESET_MODELS.length; i++) {
      if (!PRESET_MODELS[i].disabled && PRESET_MODELS[i].value && PRESET_MODELS[i].value === model)
        return true;
    }
    return false;
  },
  ...
  _renderForm: function () {
    ...
    for (var i = 0; i < PRESET_MODELS.length; i++) {
      ...
      optionsHtml +=
        '<option value="' +
        KwUtils.escapeAttr(p.value) +
        '"' +
        selected +
        disabled +
        '>' +
        KwUtils.escapeHtml(p.label) +
        '</option>';
    }
    ...
    body.innerHTML =
      ...
      '  <input id="settingBaseUrl" class="form-input" type="text" value="' +
      KwUtils.escapeAttr(config.apiBaseUrl) +
      '" placeholder="https://api.minimaxi.com/v1">' +
      '</div>' +
      '<div class="form-group">' +
      '  <label class="form-label">API Key</label>' +
      '  <input id="settingApiKey" class="form-input" type="password" value="' +
      KwUtils.escapeAttr(config.apiKey) +
      ...
      '  <select id="settingModelSelect" class="form-input">' +
      optionsHtml +
      '</select>' +
      '  <input id="settingModelCustom" class="form-input" type="text" placeholder="输入自定义模型名称" style="margin-top:6px;' +
      (isPreset ? ' display:none;' : '') +
      '" value="' +
      (isPreset ? '' : KwUtils.escapeAttr(currentModel)) +
      '">' +
      '</div>' +
      '<div class="form-group">' +
      '  <label class="form-label">系统提示词 (System Prompt)</label>' +
      '  <textarea id="settingPrompt" class="form-input form-textarea" rows="4" placeholder="你是一个AI写作助手（开悟）...">' +
      KwUtils.escapeHtml(config.systemPrompt) +
      '</textarea>' +
      '</div>' +
      '<div class="form-group">' +
      '  <label class="form-label">温度 (Temperature): <span id="tempValue">' +
      config.temperature +
      '</span></label>' +
      '  <input id="settingTemp" class="form-range" type="range" min="0" max="2" step="0.1" value="' +
      config.temperature +
      '">' +
      '</div>' +
      '<div class="form-group">' +
      '  <label class="form-label">最大 Token 数</label>' +
      '  <input id="settingMaxTokens" class="form-input" type="number" value="' +
      config.maxTokens +
      ...
      '    <input id="opencodeUrlInput" class="form-input" type="text" value="' +
      KwUtils.escapeAttr(config.opencodeUrl || '') +
      '" placeholder="http://127.0.0.1:4096">' +
      '  </div>' +
      '  <div class="form-group">' +
      '    <label class="form-label">用户名</label>' +
      '    <input id="opencodeUsernameInput" class="form-input" type="text" value="' +
      KwUtils.escapeAttr(config.opencodeUsername || '') +
      '" placeholder="opencode">' +
      '  </div>' +
      '  <div class="form-group">' +
      '    <label class="form-label">密码</label>' +
      '    <input id="opencodePasswordInput" class="form-input" type="password" value="' +
      KwUtils.escapeAttr(config.opencodePassword || '') +
    ...
    if (agentSelect) {
      ...
      agentSelect.addEventListener('change', function () {
        ...
        if (newAgent === 'build') {
          if (
            !confirm(
              '⚠️ 警告：build agent 可以执行 bash 命令和编辑文件。\n\n如果您不知道这是什么，请保持 plan agent。\n\n确定要使用 build agent 吗？'
            )
          ) {
