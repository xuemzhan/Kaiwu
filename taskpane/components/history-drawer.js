...
var HistoryDrawer = {
  ...
  _bindSearchEvents: function () {
    ...
    search.addEventListener('input', function () {
      self._filterText = String(this.value || '')
        .toLowerCase()
        .trim();
      // 过滤条件变了, 强制重新渲染
  ...
  _renderGroup: function (group) {
    ...
    var sourceLabel =
      group.sourceType === 'document' ? '全文' : group.sourceType === 'user' ? '输入' : '选区';
    var html =
      '' +
      '<section class="history-group">' +
      '  <header class="history-group-header">' +
      '    <div class="history-group-title">' +
      KwUtils.escapeHtml(group.documentRef) +
      ' · ' +
      sourceLabel +
      '</div>' +
  ...
  _renderEntry: function (entry) {
    ...
    return (
      '' +
      '<article class="history-entry" data-entry-id="' +
      KwUtils.escapeAttr(entry.id) +
      '">' +
      '  <div class="history-entry-head">' +
      '    <div class="history-entry-label">' +
      '      <span>' +
      KwUtils.escapeHtml(entry.actionLabel) +
      '</span>' +
      '      <span class="history-entry-status ' +
      statusClass +
      '">' +
      this._statusText(entry.status) +
      '</span>' +
      '    </div>' +
      '    <div class="history-entry-time">' +
      KwUtils.formatTime(entry.updatedAt || entry.createdAt) +
      '</div>' +
      '  </div>' +
      (sourcePreview
        ? '<div class="history-entry-source">' + KwUtils.escapeHtml(sourcePreview) + '</div>'
        : '') +
      '  <div class="history-entry-preview">' +
      KwUtils.escapeHtml(resultPreview) +
      '</div>' +
  ...
  _statusText: function (status) {
    return (
      { pending: '等待', streaming: '生成', done: '完成', error: '失败', cancelled: '已取消' }[
        status
      ] ||
      status ||
      ''
