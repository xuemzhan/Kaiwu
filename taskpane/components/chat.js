...
var ChatUI = {
  ...
  _bindVisibility: function () {
    ...
    document.addEventListener('visibilitychange', function () {
      if (document.hidden) {
        if (self._actionCheckTimer) {
          clearInterval(self._actionCheckTimer);
          self._actionCheckTimer = null;
        }
        if (self._contextTimer) {
          clearInterval(self._contextTimer);
          self._contextTimer = null;
        }
        if (self._selectionCheckTimer) {
          clearInterval(self._selectionCheckTimer);
          self._selectionCheckTimer = null;
        }
      } else {
        if (!self._actionCheckTimer)
          self._actionCheckTimer = setInterval(self._checkPendingAction.bind(self), 1000);
        if (!self._contextTimer)
          self._contextTimer = setInterval(self._updateContextBar.bind(self), 1500);
        if (!self._selectionCheckTimer)
          self._selectionCheckTimer = setInterval(self._updateChipStates.bind(self), 1000);
  ...
  sendMessage: function () {
    ...
    // 发送流式请求
    var self = this;
    this._streamController = AIService.sendStream(
      messages,
      // onChunk
  ...
  _updateWelcomeByComponent: function (componentType, componentLabel) {
    ...
    switch (componentType) {
      case 'wps':
        tip =
          '💡 提示：选中文档文字后，点击功能区按钮可进行润色、续写、翻译、摘要、扩写、改写等操作。';
        break;
      case 'et':
  ...
  _updateContextBar: function () {
    ...
    try {
      if (window.Application && window.Application.PluginStorage) {
        var stored = parseInt(
          window.Application.PluginStorage.getItem('taskpane_user_width') || '0',
          10
        );
  ...
  _bindEvents: function () {
    ...
    // 发送按钮
    safeBind('btnSend', 'click', function () {
      self.sendMessage();
    });

    // 停止生成按钮
    safeBind('btnAbort', 'click', function () {
      self.abortStreaming();
    });
    ...
    // 清空按钮
    safeBind('btnClear', 'click', function () {
      self.clearChat();
    });

    // 新建对话
    safeBind('btnNewChat', 'click', function () {
      self.newChat();
    });
    ...
    // 插入助手消息
    safeBind('btnExecuteAction', 'click', function () {
      self._executeInsertLastAssistant();
    });

    // 取消快捷操作
    safeBind('btnCancelAction', 'click', function () {
    ...
    for (var i = chat.messages.length - 1; i >= 0; i--) {
      if (chat.messages[i].role === 'assistant') {
        ...
        if (window.__WPS_BRIDGE__ && window.__WPS_BRIDGE__.insertContent) {
          ...
          var componentLabel =
            typeof ComponentDetector !== 'undefined'
  ...
  _updateTokenCounter: function () {
    ...
    if (chars === 0) {
      ...
      counter.textContent =
        KwUtils.formatNumber(chars) + ' 字 · 约 ' + KwUtils.formatNumber(tokens) + ' tokens';
    }
  },
  ...
  _bindRetryButton: function () {
    ...
    retryBtn.addEventListener('click', function () {
      ...
      OpenCodeAIService.testConnection(
        function (info) {
          self._updateOpencodeStatus(
            'connected',
            '已连接: ' + (Config.get('opencodeUrl') || 'http://127.0.0.1:4096')
          );
