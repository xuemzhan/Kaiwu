...
var ResultPanel = {
  ...
  _icons: {
    insert:
      '<svg viewBox="0 0 16 16" width="13" height="13" aria-hidden="true"><path d="M11 2H3a1 1 0 0 0-1 1v10a1 1 0 0 0 1 1h8l3-3V3a1 1 0 0 0-1-1h-2zm-1 7H4V8h6v1zm0-3H4V5h6v1zm2 4l2-2v6H4v-2h7a1 1 0 0 0 1-1v-1z" fill="currentColor"/></svg>',
    ...
    clear:
      '<svg viewBox="0 0 16 16" width="13" height="13" aria-hidden="true"><path d="M3 4h10M6.5 4V2.5h3V4M5 4l.5 9a1 1 0 0 0 1 1h3a1 1 0 0 0 1-1L11 4" fill="none" stroke="currentColor" stroke-width="1.4" stroke-linecap="round" stroke-linejoin="round"/></svg>',
    replace:
      '<svg viewBox="0 0 16 16" width="13" height="13" aria-hidden="true"><path d="M3 8a5 5 0 0 1 8.5-3.5L13 6M3 8a5 5 0 0 0 8.5 3.5L13 10M13 3v3h-3M3 13v-3h3" fill="none" stroke="currentColor" stroke-width="1.4" stroke-linecap="round" stroke-linejoin="round"/></svg>',
    regenerate:
      '<svg viewBox="0 0 16 16" width="13" height="13" aria-hidden="true"><path d="M13 8a5 5 0 1 1-1.5-3.5L13 6M13 2v4h-4" fill="none" stroke="currentColor" stroke-width="1.4" stroke-linecap="round" stroke-linejoin="round"/></svg>',
    abort:
      '<svg viewBox="0 0 16 16" width="13" height="13" aria-hidden="true"><rect x="3" y="3" width="10" height="10" rx="1.5" fill="currentColor"/></svg>',
  },
  ...
  _renderCard: function (card, options) {
    ...
    if (card.status === 'error') {
      contentHtml =
        '<div class="result-error">' + KwUtils.escapeHtml(card.error || '生成失败') + '</div>';
    } else if (card.status === 'cancelled') {
      contentHtml =
        '<div class="result-error result-cancelled">' +
        KwUtils.escapeHtml(card.error || '已取消') +
        '</div>';
    ...
    var headerHtml =
      options.showHeader !== false
        ? '<div class="result-card-header">' +
          ...
          '    <button class="result-icon-btn result-icon-btn-primary" title="插入到文档" ' +
          insertDisabled +
          ' data-kw-action="insert">' +
          this._icons.insert +
          '</button>' +
  ...
  copy: function () {
    if (!this._activeCard || !this._activeCard.resultText) return;
    var text = this._cleanResult(this._activeCard.resultText);
    KwUtils.copyToClipboard(text).then(
      function () {
        KwToast.show('已复制');
      },
      function () {
  ...
  regenerate: function () {
    ...
    var self = this;
    setTimeout(function () {
      self._regenerateInFlight = false;
    }, 800);
    ...
    try {
      if (typeof window !== 'undefined' && window.dispatchEvent) {
        window.dispatchEvent(
          new window.CustomEvent('kwresult:cleared', {
