...
var OpenCodeAIService = {
  ...
  _request: function (method, path, body, options, onSuccess, onError) {
    ...
    var timeoutId = null;
    // settled guards against double-firing callbacks when Promise.race
    // resolves with the losing branch's microtask AFTER the race already
    // settled. Without it, a slow fetch whose body parse rejects AFTER
    // the timeout fires would still call onError a second time. We also
    // use it to guarantee onError-or-onSuccess but never both.
    var settled = false;
    var fireSuccess = function (data) {
      if (settled) return;
      settled = true;
    ...
    var self = this;
    var fetchPromise = fetch(url, fetchOptions).then(
      function (response) {
        ...
        return response.json().then(
          ...
          function (err) {
            fireError({ message: 'JSON parse error: ' + err.message, retryable: false });
            return null;
          }
        );
      },
      function (err) {
        fireError({ message: 'Network error: ' + err.message, retryable: true });
        return null;
      }
    );

    Promise.race([fetchPromise, timeoutPromise]).catch(function () {
      // Whichever branch lost the race, the winner either fired a
      // callback (so settled=true and the loser's microtask is a
      // no-op) OR is the timeout branch itself. The timeout promise
      // never fires onError on its own — we must do it here.
      if (!settled) {
        fireError({ message: 'Request timeout', retryable: true });
      }
    });
  },
  ...
  testConnection: function (onSuccess, onError) {
    ...
    fetch(url, {
      ...
      .catch(function (err) {
        onError &&
          onError({ message: err.message || '无法连接到服务器，请检查 opencode 是否运行' });
      });
  },
  ...
  mapModifyAction: function (actionId, text, context) {
    var modifyPrompts = {
      polish_quick: {
        ...
        user:
          '请对以下文字进行润色，保持原意，提升表达质量和可读性。只返回润色后的正文：\n\n' +
          (text || ''),
      },
      polish_formal: {
        system: '你是一个正式文书写作助手，擅长办公、公文和商务表达。',
        user:
          '请将以下文字改写得更加正式、严谨、适合办公文档。保持原意，只返回改写后的正文：\n\n' +
          (text || ''),
      },
      polish_government: {
        system: '你熟悉党政机关、公文材料和政务表达风格。',
        user:
          '请将以下文字调整为更符合党政机关、公文材料的表达风格。保持原意，只返回正文：\n\n' +
          (text || ''),
      },
      correct: {
        system: '你是一个中文校对与纠错助手。',
        user:
          '请修正以下文字中的错别字、病句、标点和明显语法问题。保持原意，只返回修正后的正文：\n\n' +
          (text || ''),
      },
      expand: {
        system: '你是一个内容扩写助手，擅长补充细节、论据和表达层次。',
        user:
          '请对以下内容进行扩写，增加细节、论据和表达层次，保持原文观点不变。只返回扩写后的正文：\n\n' +
          (text || ''),
      },
      shrink: {
      ...
      rewrite: {
        ...
        user:
          '请改写以下文字，优化表达方式和句式结构，提升可读性，但保持原意不变。只返回改写后的正文：\n\n' +
          (text || ''),
      },
      translate: {
        system: '你是一个专业翻译助手。',
        user:
          '请翻译以下文字。如果原文是中文，请翻译成英文；如果原文不是中文，请翻译成中文。只返回译文：\n\n' +
          (text || ''),
      },
    };
  ...
  mapLayoutAction: function (actionId, documentText, options) {
    var layoutPrompts = {
      ai_layout: {
        ...
        user:
          '请将以下内容按照标准论文格式重新排版，包含：标题、摘要、关键词、正文、参考文献。只返回排版后的Markdown内容：\n\n' +
          (documentText || ''),
      },
      mindmap: {
        system: '你是一个思维导图生成助手，擅长将文档内容转化为Mermaid格式的思维导图。',
        user:
          '请将以下文档内容转化为Mermaid mindmap语法格式的思维导图。只返回Mermaid代码：\n\n```mermaid\nmindmap\n' +
          (documentText || '') +
          '\n```',
      },
      doc_to_ppt: {
        ...
        user:
          '请分析以下文档内容，生成一个5-10页的PPT大纲。每页包含：\n- 页面标题\n- 3-5个要点（简洁、有信息量）\n- 建议的视觉元素（图表/图片/表格）\n\n文档内容：\n' +
          (documentText || ''),
      },
    };
  ...
  mapDocumentAction: function (actionId, documentText, query) {
    var docPrompts = {
      summarize: {
        system: '你是一个文档摘要助手。',
        user:
          '请为以下文字生成简洁摘要，提炼核心要点。输出应适合插入文档：\n\n' + (documentText || ''),
      },
      doc_summary: {
        system: '你是一个文档总结助手，擅长从长文档中提炼结构和结论。',
        user:
          '请总结以下文档内容，按"一句话总结、核心要点、结构大纲、行动建议"输出：\n\n' +
          (documentText || ''),
      },
      doc_qa: {
        system:
      ...
      talk_doc: {
        ...
        user:
          '请将以下文档内容改写为适合朗读讲解的脚本，使用自然流畅的口语化表达，保留核心信息：\n\n' +
          (documentText || ''),
      },
    };
