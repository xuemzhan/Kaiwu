...
var DEFAULT_API_KEY = (function () {
  try {
    if (
      ...
      typeof window.__ENV_API_KEY__ !== 'undefined' &&
      window.__ENV_API_KEY__
    )
      return window.__ENV_API_KEY__;
  } catch (e) {
    /* ignore */
  }
  return '';
...
var DEFAULT_API_BASE = (function () {
  try {
    if (
      typeof window !== 'undefined' &&
      typeof window.__ENV_API_BASE__ !== 'undefined' &&
      window.__ENV_API_BASE__
    )
      return window.__ENV_API_BASE__;
  } catch (e) {
    /* ignore */
  }
  return '';
...
var DEFAULT_MODEL = (function () {
  try {
    if (
      typeof window !== 'undefined' &&
      typeof window.__ENV_MODEL__ !== 'undefined' &&
      window.__ENV_MODEL__
    )
      return window.__ENV_MODEL__;
  } catch (e) {
    /* ignore */
  }
  return '';
...
var Config = {
  _defaults: {
    ...
    model: 'MiniMax-M3',
    systemPrompt:
      '你是一个专业的AI助手（开悟），帮助用户高效处理文档、表格、演示和PDF。请用中文回复。',
    temperature: 0.7,
    maxTokens: 4096,
    stream: true,
  ...
  init: function () {
    ...
    if (
      (!this._data.apiBaseUrl || this._data.apiBaseUrl === this._defaults.apiBaseUrl) &&
      typeof DEFAULT_API_BASE !== 'undefined' &&
      DEFAULT_API_BASE
    ) {
      this._data.apiBaseUrl = DEFAULT_API_BASE;
    }
    if (
      (!this._data.model || this._data.model === this._defaults.model) &&
      typeof DEFAULT_MODEL !== 'undefined' &&
      DEFAULT_MODEL
    ) {
