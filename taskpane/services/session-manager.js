...
var SessionManager = {
  ...
  create: function (documentId, options, onSuccess, onError) {
    ...
    var authHeader =
      'Basic ' +
      btoa((config.opencodeUsername || 'opencode') + ':' + (config.opencodePassword || ''));
  ...
  delete: function (sessionId, onSuccess, onError) {
    var config = Config.getAll();
    var url =
      (config.opencodeUrl || 'http://127.0.0.1:4096').replace(/\/+$/, '') + '/session/' + sessionId;
    var authHeader =
      'Basic ' +
      btoa((config.opencodeUsername || 'opencode') + ':' + (config.opencodePassword || ''));
  ...
  abort: function (sessionId, onSuccess, onError) {
    var config = Config.getAll();
    var url =
      (config.opencodeUrl || 'http://127.0.0.1:4096').replace(/\/+$/, '') +
      '/session/' +
      sessionId +
      '/abort';
    var authHeader =
