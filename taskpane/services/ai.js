  ...
  _classifyError: function (status, rawText) {
    ...
    switch (status) {
      case 401:
        return {
          message:
            'API Key 无效或已过期 (401)。请到 ⚙️ 设置检查 API Key。' + (apiMsg ? ' ' + apiMsg : ''),
          retryable: false,
        };
      case 403:
        return {
          message: 'API Key 权限不足 (403)。' + (apiMsg ? ' ' + apiMsg : ''),
          retryable: false,
        };
      case 404:
        return {
          message:
            'API 地址或模型名称不存在 (404)。请检查 Base URL 和 Model 名称。' +
            (apiMsg ? ' ' + apiMsg : ''),
          retryable: false,
        };
      case 408:
      ...
      case 429:
        return {
          message: '请求过于频繁 (429)，已达速率上限。' + (apiMsg ? ' ' + apiMsg : ''),
          retryable: true,
        };
      case 500:
      case 502:
