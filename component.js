...
var ComponentDetector = {
  ...
  bindAutoReset: function () {
    ...
    try {
      ...
      this._bound = true;
      app.ApiEvent.AddApiEventListener('DocumentOpen', function () {
        self.reset();
      });
      app.ApiEvent.AddApiEventListener('NewDocument', function () {
        self.reset();
      });
      if (typeof app.ApiEvent.AddApiEventListener === 'function') {
        try {
          app.ApiEvent.AddApiEventListener('PresentationNewSlide', function () {
            self.reset();
          });
        } catch (e) {
          /* not WPP */
