...
function _initWakeWord() {
  ...
  try {
    ...
    console.debug('[Ribbon] WakeWord 初始化失败:', e);
  }
}
...
function readSelectionText() {
  try {
    ...
    var sel =
      app.Selection ||
      (app.ActiveDocument &&
        app.ActiveDocument.Application &&
        app.ActiveDocument.Application.Selection) ||
      (app.ActiveDocument && app.ActiveDocument.Selection) ||
      (app.ActiveWorkbook &&
        app.ActiveWorkbook.Application &&
        app.ActiveWorkbook.Application.Selection) ||
      (app.ActiveWorkbook && app.ActiveWorkbook.Selection);
    if (sel && sel.Text) return String(sel.Text).replace(/\r$/, '').trim();
  } catch (e) {
    console.debug('[Ribbon] 读取选区文本失败:', e);
  }
  return '';
}
  ...
  try {
    ...
    try {
      if (
        ...
        typeof window.Application.ApiEvent.AddApiEventListener === 'function'
      ) {
        var onSel = function () {
          invalidateRibbonCaches();
        };
      ...
      console.debug('[Ribbon] 注册事件监听失败:', e4);
    }

    try {
      if (ribbonUI && typeof ribbonUI.Invalidate === 'function') ribbonUI.Invalidate();
    } catch (e2) {
      console.debug('[Ribbon] 刷新 Ribbon 失败:', e2);
    }
    _initWakeWord();
    return true;
  } catch (error) {
...
var TaskPaneManager = {
  ...
  getOrCreate: function () {
    ...
    try {
      ...
      console.debug('[TaskPane] 获取 TaskPane ID 失败:', e);
    }
    ...
    if (tsId) {
      try {
        ...
        console.debug('[TaskPane] 获取 TaskPane 失败:', e2);
      }
    }
    ...
    try {
      ...
      console.debug('[TaskPane] 设置初始可见性失败:', e4);
    }
    try {
      window.Application.PluginStorage.setItem('taskpane_id', newPane.ID);
    } catch (e3) {
      console.debug('[TaskPane] 保存 TaskPane ID 失败:', e3);
    }
    return newPane;
  },
  ...
  _dock: function (pane) {
    ...
    try {
      ...
      console.debug('[TaskPane] 停靠 TaskPane 失败:', e);
    }
  },
  ...
  _saveUserWidth: function (width) {
    try {
      ...
      console.debug('[TaskPane] 保存宽度失败:', e);
    }
  },
  ...
  _loadUserWidth: function () {
    try {
      ...
      console.debug('[TaskPane] 加载宽度失败:', e);
    }
    return null;
  },
  ...
  _saveVisible: function (visible) {
    try {
      ...
      console.debug('[TaskPane] 保存可见性失败:', e);
    }
  },
};
...
var FloatingAssistantManager = {
  ...
  _loadSize: function (key, min, max) {
    try {
      ...
      console.debug('[Floating] 加载尺寸失败:', e);
    }
    return null;
  },
  ...
  _isDuplicateOpen: function () {
    try {
      if (window.Application && window.Application.PluginStorage) {
        var last = parseInt(
          window.Application.PluginStorage.getItem('floating_opened_at') || '0',
          10
        );
        var now = Date.now();
      ...
      console.debug('[Floating] 检查重复打开失败:', e);
    }
    return false;
  },
  ...
  _saveContext: function (source, actionId) {
    ...
    try {
      ...
      window.Application.PluginStorage.setItem(
        'floating_selected_text',
        context.selectedText || ''
      );
      window.Application.PluginStorage.setItem(
        'floating_anchor',
        JSON.stringify(context.anchor || {})
      );
      ...
      console.debug('[Floating] 保存上下文失败:', e);
    }
  },
};
...
function collectWriterContext() {
  ...
  try {
    ...
    console.debug('[Ribbon] 收集 Writer 上下文失败:', e);
  }
  return result;
}
